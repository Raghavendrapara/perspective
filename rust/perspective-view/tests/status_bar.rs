...
use perspective_view::js_object;
use perspective_view::status_bar::StatusBarElement;
use perspective_view::utils::*;
...
fn create_element() -> (web_sys::Element, StatusBarElement) {
    ...
    let status_bar =
        StatusBarElement::new(elem.clone().unchecked_into(), "".to_owned()).unwrap();
    (elem, status_bar)
}
...
extern "C" {
    ...
    type PerspectiveJsWorker;

    #[wasm_bindgen(method)]
    pub fn table(this: &PerspectiveJsWorker, data: js_sys::Object) -> js_sys::Promise;
}
...
pub fn test_html() {
    ...
    assert_eq!(txt, "<style></style><div class=\"section\"><span class=\"\" id=\"status\"></span></div><div class=\"section\"><span class=\"button\" id=\"reset\"><span>Reset</span></span><span class=\"button\" id=\"export\"><span>Export</span></span><span class=\"button\" id=\"copy\"><span>Copy</span></span></div><div class=\"section\" id=\"rows\"><span>0 rows</span></div>");
}
...
pub async fn test_rows_count() {
    ...
    let (elem, status_bar) = create_element();
    ...
    let table = JsFuture::from(worker.table(js_object!(
        "A",
