...
impl StatusBarElement {
    ...
    pub fn new(elem: HtmlElement, css: String) -> Result<StatusBarElement, JsValue> {
        let _self = {
    ...
    ///
    /// # TODO
    /// `async_method_to_jsfunction` passed ownership of the closure to Javascript,
    /// and when this `View` is deleted the closure leaks.  
    pub fn set_view(&self, view: PerspectiveJsView) -> Result<Promise, JsValue> {
        view.on_update(self.async_method_to_jsfunction(|this| async move {
