...
fn copy_async(view: &PerspectiveJsView) -> impl Future<Output = JsResult<JsString>> {
    let csv = view.to_csv(js_object!("formatted", true));
    async move { Ok(csv.await.unwrap()) }
}
